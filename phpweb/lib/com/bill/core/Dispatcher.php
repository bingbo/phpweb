...
class Dispatcher {
    ...
    public static function dispatch() {
        ...
        if(!empty($_SERVER['PATH_INFO'])) {
            ...
            if(!self::routerCheck()){   // 检测路由规则 如果没有则按默认规则调度URL
                ...
                if(!isset($_GET[C('VAR_CONTROLLER')])) {// 还没有定义控制器名称
                    $var[C('VAR_CONTROLLER')]  =   array_shift($paths);
                }
     ...
     * @return string
     +----------------------------------------------------------
     */
    private static function getController($var) {
        $controller = (!empty($_GET[$var])? $_GET[$var]:C('DEFAULT_CONTROLLER'));
        unset($_GET[$var]);
        /*if(C('URL_CASE_INSENSITIVE')) {
            // URL地址不区分大小写
            define('P_MODULE_NAME',strtolower($controller));
